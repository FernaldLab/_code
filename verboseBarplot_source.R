verboseBarplot_1.25.2 = function (x, g, main = "", xlab = NA, ylab = NA, cex = 1, cex.axis = 1.5, 
    ...
    horiz = FALSE, ...) 
{
    ...
    err.bp = function(dd, error, two.sided = FALSE, numberStandardErrors, 
        horiz = FALSE) {print(dd); print(error)
        if (!is.numeric(dd)) {
    ...
    if (length(unique(x)) > 2) {
        ...
        p1 = tryCatch(fisher.test(x, g, alternative = "two.sided")$p.value, 
            error = function(e) {
                NA
            })
    }
