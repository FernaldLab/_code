...
# probability matrices for each group. If <byTotal>, the probability is (sum of probabilities for each fish) / (num fish);
# otherwise, (default) the probability is (sum of probabilities for each fish) / (num fish who performed <leader> at all).
# Notice that the combined probability is (probability for 1st subj + probability for 2nd subj + ...) / (number of subjects), NOT
# (total # of transitions from leader to follower in all logs) / (total # of occurrances of leader in all logs).
# This is recommended so that each row will add up to 1.
...
.behavioralDensityGraph = function(data, behaviorsToPlotAndColors, centerBeh, filename, lim = 15, noRepCenterBeh = TRUE, multifish = FALSE, timesPerBin = 0.5, ymax = 1) {
	jpeg(filename = filename, width = 15, height = 5, units = "in", quality = 100, res = 150, type = "quartz");
	...
	for (i in 1:(dim(behaviorsToPlotAndColors)[1])) {
		...
		#count <- x$varCount;
		count <- sum(unlist(lapply(data, function (d) {length(d$behavior)})));
...
.behavioralDensityGraphs = function(data, behaviorsToPlotAndColors, filePref, targetBeh = NULL, ...) {
	...
	dataByGroup = list();
	for (i in 1:length(groupNames)) {
		dataByGroup[[i]] = data[grepl(paste("^", groupNames[i], sep = ""), names(data))];
	...
	for (beh in behnames) {
		...
		for (i in 1:length(groupNames)) {
			.behavioralDensityGraph(dataByGroup[[i]], behaviorsToPlotAndColors, centerBeh = beh, filename = paste(filePref, "_", beh, "_", groupNames[i], ".jpeg", sep = ""), multifish = TRUE, ...);
		}
...
.buildDotFile = function (probMatrix, originalDataVec, file='', title='untitled', fontsize=24, minValForLine = 0, weird = FALSE, singleCharLabels = FALSE, byTotal = FALSE) { # TODO change default for singleCharLabels
