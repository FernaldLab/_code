...
# 
# outlierSamples computes inter-sample correlations and performs hierarchical clustering to find sample outliers as taught by Mike Oldham, 
...
# checkEqual checks to see if two large data frames are exactly the same
checkEqual = function(data1,data2) {
	args=match.call();
	for (row in 1:nrow(data1)) {
		
		check=sum(data1[row,] %in% data2[row,]);
		if (check==ncol(data1)) {
			cat('row',row,'ok\n');
			} else {
				cat('!!! mismatch at row',row,' !!!\n\n');
				print(args[2]);cat('row',row,':\n');
				print(data1[row,]);
				cat('\n');
				print(args[3]);cat('row',row,':\n');
				print(data2[row,]);
				break;
				}
		}
...
removeTooManyNAs = function (data, probe_thresh=5, sample_thresh=10000) {
	
	countNAs = apply(is.na(data), 1, sum);
	probes_over_thresh = (1:dim(data)[1])[countNAs > probe_thresh];
	cat('\n');
	cat(length(probes_over_thresh),'probes with >',probe_thresh,'NAs across samples \n');
	
	countNAsSamps = apply(is.na(data), 2, sum);
	samples_over_thresh = (1:dim(data)[2])[countNAsSamps > sample_thresh]; 
	cat(length(samples_over_thresh),'samples with >',sample_thresh,'NAs across probes \n');
...
outlierSamplesIterate2 = function (data, IACthresh=2, showplots=T) {
...
# # outlierSamplesIterate = function (data, IACthresh=2, showplots=T) {
	...
	# # while (TRUE) {
		...
		# # cat('Choose one (0 to quit): ')
		# # choices = c('Remove all possible outliers',
					# # 'Remove some possible outliers',
					# # 'Change IAC threshold and restart'
					# # );
		# # choose = menu(choices);
		
		# # if (choose==0) {stop('User terminated function')};
		# # if (choose==1) {
			# # temp = out$dataClean;
			# # samples_removed = c(samples_removed, to_remove);
			# # cat('Sample(s)', names(to_remove), 'removed \n');
			# # };
		# # if (choose==2) {
			# # answer = readline(prompt='List sample(s) to remove with single spaces in between (no commas): ');
			# # answer = strsplit(answer, ' '); answer = answer[[1]]; 
			# # temp = temp[, -match(answer, names(temp))];
			# # samples_removed = c(samples_removed, to_remove[names(to_remove)==answer]);
			# # };
		# # if (choose==3) {
			# # IACthresh = readline(prompt='New IAC threshold: ');
			# # temp = data;
			# # };
...
preProcess = function (datIN,
					removeOutlierProbes=T, deviate=2.5, rowORcol=1,
					removeTooManyNAs=T, probe_thresh=5, sample_thresh=10000,
	...
	if (removeOutlierSamples) {
		outlierSamplesOUTPUT = outlierSamplesIterate2(temp, IACthresh, showplots);
